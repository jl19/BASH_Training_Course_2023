...
```
DESeq2::plotMA(res_24h_vs_Control, ylim=c(-2,2))
```
...
ylim <- c(-2.5,2.5)
resGA <- results(dds_txi_stringtie, lfcThreshold=.5, altHypothesis="greaterAbs")
resLA <- results(dds_txi_stringtie, lfcThreshold=.5, altHypothesis="lessAbs")
resG <- results(dds_txi_stringtie, lfcThreshold=.5, altHypothesis="greater")
resL <- results(dds_txi_stringtie, lfcThreshold=.5, altHypothesis="less")
